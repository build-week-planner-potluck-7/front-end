...
export default function EventForm() {
  ...
  const getEvents = () => {
    axios
      .get('https://lambda-build-week.herokuapp.com/potlucks')
      .then((resp) => {
        setEvents(resp.data);
      })
  ...
  const postNewEvent = (newEvent) => {
    axios
      .post('https://lambda-build-week.herokuapp.com/potlucks', newEvent)
      .then((resp) => {
  ...
  return (
    <form>
      <div className='form'>
        ...
        <Link to='./foodForm' className='formLink' onClick={submitForm}>
          Add Food
        </Link>
      </div>
    </form>
